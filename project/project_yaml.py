...
class ProjectYaml(BaseModel):
    ...
    sanitizers: dict[str, str]  # Mapping from sanitizer IDs ("id_NNN") to crash strings.
    ...
    @property
    def all_sanitizer_ids(self) -> list[str]:
        """all sanitizers keys as list"""
        return list(self.harnesses.keys())

    def sanitizer_id_by_value(self, name: str) -> str:
        """get the sanitizer id by knowing the name of the sanitizer"""
        return self.__search_dict_for_value(self.sanitizers, name)
    ...
    @staticmethod
    def __search_dict_for_value(d: dict, to_search: Any, comparator=lambda val, query: val == query) -> Any:
